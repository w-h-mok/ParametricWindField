...
Usage:
    python scripts/pressure_comparison_ELLIP.py --station HKO_AWS
"""
...
def create_pressure_comparison_plot(station='HKO_AWS'):
    """
    ...
    # Get available TCs in order
    available_tcs = [tc for tc in TC_ORDER if tc in data['tc_name'].unique()]
    
    if len(available_tcs) == 0:
        print(f"No TCs found for station {station}")
        return
    
    print(f"Available TCs: {available_tcs}")
    ...
    # Create 4x2 grid
    fig, axes = plt.subplots(2, 4, figsize=(24, 14), sharex=False, sharey=False)
    axes = axes.flatten()
    ...
    # Plot each TC
    for idx, tc_name in enumerate(available_tcs[:8]):
        ax = axes[idx]
        ...
        # Plot ELLIP (1010) - green dashed line
    ...
    for idx in range(len(available_tcs[:8]), 8):
    ...
    fig.legend(ordered_handles, ordered_labels, loc='lower center', 
               ncol=len(ordered_handles), bbox_to_anchor=(0.5, -0.02),
               frameon=True, fontsize=20)
    
    # Add main title
    fig.suptitle(f'Pressure Time Series at {location} - ELLIP Model Comparison\nControl vs. P$_{{ambi}}$=1010 hPa', 
                 fontsize=28, fontweight='bold', y=0.98)
    
    # Adjust layout
    plt.subplots_adjust(top=0.92, bottom=0.12, left=0.06, right=0.98, 
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Plot pressure comparison for ELLIP model')
    ...
    create_pressure_comparison_plot(station=args.station)
